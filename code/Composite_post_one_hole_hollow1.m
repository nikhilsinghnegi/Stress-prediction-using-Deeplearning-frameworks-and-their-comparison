...
folder_path = 'D:\8th sem\BTP_II\DiNN-framework-Stress-Prediction-for-Heterogeneous-Media-master\Data processing\Plate with circular cutout model\check_here\';
...
%% Parameters configuration
    ...
    Cart_data = Cartesian_Map(rectangle_x, rectangle_y, n_x, n_y); %coordinate values of 
    ...
    for mm = 1:length(stress(:,3))
        stress_map(mm,1) = sqrt(stress(mm,3)^2+stress(mm,4)^2-stress(mm,3)*stress(mm,4)+3*stress(mm,6)^2); %stress formula for von mises stress
    end
...
%    Remove areas without any nodes in geometry
    ...
    for f = 1:length(Cart_dist(:,1))
        ...
        if Cart_dist(f,1) < circle_r
            ...
            stress_cart(f) = 0;%have already made them zero then what is the need of this
    ...
    figure(4*iii-1) %no need of plotting it again and again
...
%         cir = circle_dist(Cart_data(jj,1),Cart_data(jj,2),circle_x,circle_y,r);
        ...
        if Cart_dist(jj,1) < circle_r
            ...
            %Cart_dist(jj,1) = 1;
            Cart_dist(jj,1) = 0;% was 1 previously, should be 0
        elseif Cart_dist(jj,1) == circle_r
            
            %Cart_dist(jj,1) = 0;
            Cart_dist(jj,1) = 1;% was 0 previously, should be 1
        else
